/**
 * Escudos oficiales de selecciones (football-data.org).
 * Incluye nombres en inglés de la API, abreviaturas y alias en español.
 */

const CREST_BY_KEY: Readonly<Record<string, string>> = {
...
const ISO_FALLBACK: Readonly<Record<string, string>> = {
  Albania: 'al',
  'Costa Rica': 'cr',
  Denmark: 'dk',
  Georgia: 'ge',
...
function crestUrlForNombre(nombreEquipo: string): string {
  ...
  const directo = CREST_BY_KEY[nombre];

  if (directo !== undefined) {
    return directo;
  }

  const alias = ALIAS_ES[nombre];
  const porAlias = alias === undefined ? undefined : CREST_BY_KEY[alias];

  if (porAlias !== undefined) {
    return porAlias;
  }

  const iso = ISO_FALLBACK[nombre] ?? (alias === undefined ? undefined : ISO_FALLBACK[alias]);
